...
public class AdminController {
	public static void main(String[] args) {
		...
		while(true) {
			switch(JOptionPane.showInputDialog("0.exit 1.add 2.count 3.list 4.findById 5.findByName 6.updatePw")) {
			case "0" :
				return;
			case "1" :
			...
			case "3" :
				...
				break;
			case "4" :
				MemberBean result1 = adminService.findById(JOptionPane.showInputDialog("찾고 싶은 id를 입력하세요."));
				if(result1 == null) {
			...
			case "5" :
				String result2 = "";
				MemberBean[] temp = adminService.findByName(JOptionPane.showInputDialog("찾고 싶은 name을 입력하세요."));
				if(temp.length == 0) {
					result2 = "찾는 이름이 없습니다.";
				} else {
